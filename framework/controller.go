...
type Container interface {
	// Bind 绑定一个服务提供者，如果关键字凭证已经存在，会进行替换操作，返回err
	Bind(provider ServiceProvider) error
	// IsBing 关键字凭证是否已经绑定服务提供者
	IsBing(key string) bool

	// Make 根据关键字凭证获取一个服务
	Make(key string) (interface{}, error)

	// MustMake 根据关键字凭证获取一个服务，如果这个关键字凭证未绑定服务提供者，那么会panic
	//所以在使用这个接口的时候请保证服务已经为这个关键字绑定了服务提供者。
	MustMake(key string) interface{}

	// MakeNew 根据关键字凭证获取一个服务，只是这个服务并不是单例
...
func (hade *HadeContainer) IsBing(key string) bool {
...
func (hade *HadeContainer) make(key string, params []interface{}, forceNew bool) (interface{}, error) {
	...
	if sp == nil {
		return nil, errors.New("Contract " + key + " hava not register")
	}
	if forceNew {
	...
	inst, err := hade.newInstance(sp, params)
	if err != nil {
		return nil, err
	}
